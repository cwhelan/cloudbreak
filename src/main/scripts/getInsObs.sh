...
#
# This script generates insert sizes for each position in the alignment. 
# Chris Whelan & Zak Shafran, Feb 2013
#
# Note, this script is a trivial modification of the script
# cloudbreak/src/main/scripts/Cloudbreak-full.sh The script runs off
# the intermediate data that was generated by Chris Whelan and the
# corresponding sections are commented out.
# 

set -e
set -u

# to run this script, update the values in the first three sections
# with the appropriate paths and parameters for your data and file
# system
...
CLOUDBREAK_HOME=/l2/users/zak/svariants/cloudbreak
...
HDFS_EXPERIMENT_DIR=/user/whelanch/cloudbreak/venter_chr2_allindels_100bp_dip
HDFS_SANDBOX_DIR=/user/zak/sandbox
...
time hadoop jar $CLOUDBREAK_HOME/target/cloudbreak-0.2-SNAPSHOT-exe.jar -Dmapred.reduce.tasks=$GMM_REDUCE_TASKS computeInsHistogram \
   --inputFileDescriptor $HDFS_EXPERIMENT_DIR/readGroupInfo.txt \
   --outputHDFSDir $HDFS_SANDBOX_DIR/insHist/ \
